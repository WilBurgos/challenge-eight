...
class CatastroSeeder extends Seeder
{
    ...
    public function run()
    {
        $filename0 = str_replace("\\", "/", database_path() . '/data/spot2-0.sql');
        dump(\DB::unprepared(file_get_contents($filename0)));

        $filename1 = str_replace("\\", "/", database_path() . '/data/spot2-1.sql');
        dump(\DB::unprepared(file_get_contents($filename1)));
